...
import utils
from actor_and_data import BarCountDataActor
...
# Prepare type
euro_futures_1min_bar_type = BarType(
...
loaded_bars: list[Bar] = utils.load_bars_from_ninjatrader_csv(
    ...
    bar_type=euro_futures_1min_bar_type,
)

# Strategy config
config = DemoStrategyConfig(instrument=euro_futures_instrument, primary_1min_bar_type=euro_futures_1min_bar_type)

# Strategy
strategy = DemoStrategy(config)

# Actor
market_data_actor = BarCountDataActor(bar_type=euro_futures_1min_bar_type)

# Run backtest
engine: BacktestEngine = utils.run_backtest(
    strategy=strategy,
    actors=[market_data_actor],
    venue=venue,
    instrument=euro_futures_instrument,
    bars=loaded_bars,
    start=None,
